#!/bin/bash
# ============================================================
# 红酒质量预测 —— XGBoost GPU 加速完整流程
# 改进方案：XGBoost GPU加速 + 特征工程 + 加权损失 + 超参数搜索
# ============================================================
set -euo pipefail
...
echo "  XGBoost: $(python3 -c 'import xgboost; print(xgboost.__version__)')"
...
echo "  joblib: $(python3 -c 'import joblib; print(joblib.__version__)')"
echo ""
...
time python3 "${SRC_DIR}/predict.py"
...
# 从日志中提取准确率
if [ -f "${LOG_FILE}" ]; then
    ACC_LINE=$(grep -E "测试集准确率|accuracy" "${LOG_FILE}" | tail -1)
    echo "  最新准确率信息: ${ACC_LINE:-N/A}"
fi
