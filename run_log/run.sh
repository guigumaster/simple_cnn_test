...
#   3. SMOTE 过采样处理类别不平衡（平衡后所有类别样本数相同）
#   4. 二阶交互特征工程（PolynomialFeatures, interaction_only=True）
#   5. XGBoost early_stopping + best_iteration 重新拟合
#   6. 质量标签映射（3-9 → 0-6）适配 XGBoost 要求
#
# 预期效果：
#   - 准确率从基线 62.69% 提升
#   - 稀有类别（质量 3、4、8、9）的召回率改善
#
# 使用方法：
...
check_environment() {
    ...
    # 检查关键 Python 包
    # 注意：scikit-learn 的 import 名称为 sklearn，不是 scikit-learn
    log "Python 包检查:"
    for pkg_info in "xgboost:xgboost" "sklearn:scikit-learn" "pandas:pandas" "numpy:numpy" "joblib:joblib" "imblearn:imbalanced-learn"; do
        import_name="${pkg_info%%:*}"
        pkg_name="${pkg_info##*:}"
        if $PYTHON_CMD -c "import $import_name" 2>/dev/null; then
            ver=$($PYTHON_CMD -c "import $import_name; print($import_name.__version__)" 2>/dev/null || echo "unknown")
            log "  $pkg_name == $ver"
        else
            log "  $pkg_name - 未安装"
        fi
...
train_model() {
    ...
    log "  过采样: SMOTE（使用 imbalanced-learn）"
    log "  提前停止: early_stopping_rounds=50"
    log "  正则化: L1=1.0, L2=2.0, gamma=0.2"
...
print_summary() {
    ...
    log "改进方案总结:"
    log "  ✅ GPU 加速 XGBoost (device=cuda, tree_method=hist)"
    log "  ✅ 补全 wine_type 特征（红酒/白酒判别）"
    log "  ✅ SMOTE 过采样平衡类别分布"
    log "  ✅ 二阶交互特征工程（PolynomialFeatures）"
    log "  ✅ 质量标签映射（3-9 → 0-6）适配 XGBoost"
    log ""
    log "日志文件: ${LOG_FILE}"
    log "模型文件: ${MODEL_DIR}/wine_model.joblib"
